...
for item in gear_table.find_all('tbody'):
    ...
    for row in rows:
        ...
        fuel_type = row.find_all('td')[5].text.strip()
        flame_control = row.find_all('td')[6].text.strip()
        ideal_for = row.find_all('td')[7].text.strip()

        print(mainTitle + ', ' + subTitle + ', ' + msrp + ', ' + weight + ', ' + fuel_type + ', ' + flame_control + ', ' + ideal_for)
